...
public enum Sq
{
	...
	public Sq getNextSqInDirection(final Direction direction)
	{
		...
		final byte newX = (byte) (startSq.x + direction.dx);
		final byte newY = (byte) (startSq.y + direction.dy);
	 ...
	 */
	public Sq[] getAllSqInDirection(final Direction direction, final boolean includeStart, final byte lengthCap)
	{
	...
	public static Sq getSq(final byte x, final byte y)
	{
	 ...
	 * (x,y) in {0,...,7} X {0,...,7}.
	 */
	public static boolean coordinatesAreValid(final byte x, final byte y)
	{
		return 0 <= x && x < 8 && 0 <= y && y < 8;
	}

	public static Sq getSq(final int i, final int j)
	{
		return getSq((byte) i, (byte) j);
	}
