 ...
 * @date 23 Jan 2017
 * 
 * Second of three utility classes containing only
 * static methods to initialise the constants in 
 * the BBDB class. This class initialises everything
 * we need to generate the magic move databases for 
 * sliding pieces (and for the pawn first moves).
 */
public class BBDBInitialisationUtilsSection2
{
     ...
     private static long[][] generatePawnFirstMoveOccVars(final boolean isWhite)
     {
          ...
          for (byte i = 0; i < 8; i++)
          {
               final Sq[] relevantSquares = Sq.values()[shiftFactor + i].getAllSqInDirection(movementDirection, false, (byte) 2);
